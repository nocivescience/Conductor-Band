from manim import *
class WhatIsAMomentum(Scene):
        def construct(self):
                intro=Text("What is a Momentum")
                other_moments=Text("Exist Other momentums besides to the linear momentum")
                self.play(Write(intro),run_time=4)
                self.play(Transform(intro,other_moments),run_time=3)
                self.wait()
